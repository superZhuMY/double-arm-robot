...
void executeTrajectory(const control_msgs::FollowJointTrajectoryGoalConstPtr& goal, ActionServer* as)
{
	...
	if (points_count == 0 || msg.points.front().positions.size() < 6) {
        ROS_WARN("[R] Received invalid trajectory");
        as->setAborted();
        return;
    }
	...
	ROS_INFO("[R] Received trajectory points: %d", points_count);
...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "jaka_control_node_R");
	ros::NodeHandle nh;
	command_pub=nh.advertise<jaka_controller_tcp::Command>("command_R",1);
	...
	//ActionServer action_server(nh, "arm_controller/follow_joint_trajectory", boost::bind(&executeTrajectory, _1, &action_server), false);
  	ActionServer action_server(nh, "/Double_arm_robot/r_arm/follow_joint_trajectory", boost::bind(&executeTrajectory, _1, &action_server), false);
